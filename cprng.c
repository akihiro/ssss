...
int cprng_init(void)
{
  int cprng = open(RANDOM_SOURCE, O_RDONLY);
  if (cprng < 0)
    ssss_errmsg = "couldn't open " RANDOM_SOURCE;
  return cprng;
}
...
int cprng_deinit(int cprng)
{
  int ret = close(cprng);
  if (ret < 0)
    ssss_errmsg = "couldn't close " RANDOM_SOURCE;
  return ret;
}
...
int cprng_read(int cprng, uint8_t *buf, size_t len)
{
  ...
  for(count = 0; count < len; count += i)
    if ((i = read(cprng, buf + count, len - count)) < 0) {
      close(cprng);
      ssss_errmsg = "couldn't read from " RANDOM_SOURCE;
      return i;
    }
  return 0;
}
